...
function CollectionsContent() {
  ...
  const filteredCollections = useMemo(() => {
    ...
    if (selectedRating !== null) {
      filtered = filtered.filter(c => c.rating && c.rating >= selectedRating);
    }
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12">
          ...
          <div className="text-center mb-12 md:mb-16">
            ...
            <p className={`text-xl md:text-2xl max-w-3xl mx-auto leading-relaxed ${theme === 'dark' ? 'text-[#F5F3EE]/60' : 'text-[#0B0B0B]/60'}`}>
              Discover our curated fashion collections featuring stylish clothing for men and women. From classic t-shirts and shorts to elegant blouses and dresses.
            </p>
          </div>
          ...
          <div className="max-w-7xl mx-auto mb-8">
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
              ...
              >
                ...
                Filters {hasActiveFilters && `(${filteredCollections.length})`}
              </button>
              ...
              <div className="hidden sm:block">
                <p className={`text-sm font-medium ${theme === 'dark' ? 'text-[#F5F3EE]/60' : 'text-[#0B0B0B]/60'}`}>
                  Showing {filteredCollections.length} {filteredCollections.length === 1 ? 'item' : 'items'}
                </p>
              </div>
            </div>
          </div>
              ...
              {filteredCollections.length > 0 ? (
                ...
                >
                  {filteredCollections.map((collection, index) => (
                    ...
                    >
                      <ProductCard product={collection} showAddToCart={true} />
                    </div>
                  ))}
                </div>
              ) : (
      ...
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-[#0B0B0B]">
          <div className="text-center">
            <div className="text-[#D4AF37] text-xl font-semibold">Loading collections...</div>
          </div>
        </div>
      }
    >
